...
// 播放器引擎优先级：原生 HLS(Safari/iOS，免 CORS) -> hls.js(Chromium，受 CORS) -> 直链 -> 失败提示。
export default function Player({ url, sourceId, onAllFailed }: Props) {
  ...
  const playUrl =
    isHttps && wasHttp && canUp ? url.replace(/^http:\/\//, "https://") : url;
  ...
  useEffect(() => {
    ...
    // 优先原生 HLS（Safari/iOS，免 CORS）；无原生 HLS 时才用 hls.js（桌面 Chromium，受 CORS）
    ...
    return (
      <div className="player-fail">
        ...
        <p className="player-fail-hint">
          为什么手机 / Safari 能播、桌面 Chrome 不能播？因为桌面 Chromium
          没有原生 HLS，只能用 JS(hls.js) 拉流，会被 CORS / 防盗链拦截 ——
          这是浏览器机制，不是本站故障。电脑端推荐点「下载播放文件」(.strm)
          双击用 VLC / PotPlayer 播放，或点「用 VLC
          打开」；受地区限制的源需在对应地区网络下才能播。
        </p>
      </div>
    );
  }
